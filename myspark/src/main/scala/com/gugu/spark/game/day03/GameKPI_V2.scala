...
object GameKPI_V2 {
  def main(args: Array[String]): Unit = {
    ...
    val conf: SparkConf = new SparkConf().setAppName("GameKPI_V2").setMaster("local[*]")
    val sc = new SparkContext(conf)
    val lines: RDD[String] = sc.textFile("D:\\logs\\game\\")
    val splitedRDD: RDD[Array[String]] = lines.map(_.split("[|]"))
    val filtered: RDD[Array[String]] = splitedRDD.filter(fields => {
