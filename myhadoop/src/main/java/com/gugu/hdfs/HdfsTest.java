...
//        conf.set("","");// 配置信息加载顺序 先加载core-default.xml,然后加载项目中的core-site.xml，最后加载代码中的配置，后加载的覆盖前面的
        FileSystem fileSystem = FileSystem.get(new URI("hdfs://master:8020"),conf, "gugu"); // 通过fileSystem对hdfs进行操作
        fileSystem.copyFromLocalFile(new Path("D:\\LICENSE.txt"),new Path("/gugu/test"));
        fileSystem.close();
    }
}
